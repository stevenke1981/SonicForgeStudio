...
- 可編譯的純 Rust 音訊核心原型
- 可產生 WAV 的 CLI 示範程式
- 可重現的 Tauri 2 + React + TypeScript GUI vertical slice（`apps/desktop/`）
...
產物放在 `dist/`，包含 CLI binary 與 `SHA256SUMS.txt`。GitHub Actions 的 `v*` tag workflow 會建立 Windows、Ubuntu、macOS CLI artifacts，以及 Windows desktop executable；目前不包含 installer 或簽章。

## 目前驗收狀態

- Rust M0 headless baseline：已通過，完整命令與 WAV / release evidence 見 `BUILD_VERIFICATION.md`。
- GUI vertical slice：主要 M0/M5 criteria 已通過；Piano Roll、real-time audio IPC 與 DPI pass 仍未完成。完整 acceptance 見 `todos.md`、`final.md`、`BUILD_VERIFICATION.md`。
- Tauri shell 目前使用 read-only app/audio status commands；不進入 audio callback，也不讓 UI 直接持有 DSP node。
