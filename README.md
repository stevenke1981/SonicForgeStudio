...
- 可發聲的 SFX Lab 與 Mixer transport、播放完成自動停止、WAV 匯出、使用者命名的另存新檔
...
1. 進入 **Music**，在左側 Instruments 區按 **＋** 或 **Add instrument／新增樂器**。
2. 從 10 種內建音色選一種；程式會新增一條含示範音符的軌道，可直接按 **Play** 聽聲音。
3. 選取軌道後，可在右側 Inspector 的 **Track instrument／軌道樂器** 下拉選單更換音色。
4. 在時間軸空白處點擊或拖曳橘色播放頭定位；聚焦播放頭後可用方向鍵微調，Home/End 跳到頭尾。

內建音色包含 Analog Lead、Warm Pad、Electric Bass、Soft Keys、Bell、Pluck、Drum Kit、Kick、Snare、Hi-Hat，全部由 Rust DSP 即時產生，不需要額外下載音色檔。Drum Kit 使用 General MIDI 常用 note：36 Kick、38/40 Snare、42/44/46 Hi-Hat。

## SFX、Mixer、匯出與範本

- **SFX Lab**：選擇五個內建 recipe 任一項，按 **Preview／預聽** 發聲，按該卡片的 **Stop／停止** 結束。
- **Mixer**：以 Play／Pause／Stop 播放目前完整專案；調整 mute／gain 後，下次 Play 會以更新後的 Rust graph 播放。
- **Export WAV／匯出 WAV**：輸入英數字、點、連字號或底線組成的安全檔名；完成後介面顯示實際輸出路徑、frame 數與 sample rate。
- **Save As／另存新檔**：輸入新的專案名稱與安全 ID，保留原專案並切換到新副本。
- **Templates／範本**：共九個，包含空白、小星星、四拍鼓組、Chiptune、SFX Starter、Piano Ballad、Bass Groove、Synthwave、Cinematic。
...
- Desktop GUI：Piano Roll、Step Sequencer 1–64、200-op history、專案 save/load/save-as/recovery、CPAL/WASAPI transport、SFX／Mixer playback、WAV export、100–200% DPI、英文／繁中／日文／韓文與九個起始範本均已落地。完整 acceptance 見 `todos.md`、`final.md`、`BUILD_VERIFICATION.md`。
